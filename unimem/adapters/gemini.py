"""Gemini CLI agent adapter, subclassing GenericAdapter for session execution wrapper."""
...
class GeminiAdapter(GenericAdapter):
    """Adapter for wrapping Gemini CLI sessions."""

    def launch(self, command: List[str] = None) -> None:
        """Launch Gemini CLI, defaulting to standard CLI execution."""
        if not command:
            command = ["gemini"]
            
        logger.info("[cyan]Initializing Gemini CLI Unimem Adapter...[/cyan]")
        
        manager = MemoryManager(self.project_root)
        session_id = None
        if manager.is_initialized():
            session = manager.start_session("gemini")
        ...
        try:
            ...
            if session_id and manager.is_initialized():
                ...
                event = Event(
                    ...
                    prompt=f"Gemini session finished with exit code {result.returncode}",
                    response_summary=f"Gemini completed task with exit code {result.returncode}.",
                    files_changed=final_changed
                )
                manager.record_event(event)
                manager.end_session(session_id)
        except FileNotFoundError:
            logger.error("[red]Gemini command not found. Please ensure Gemini CLI is installed or specify path.[/red]")
            if session_id and manager.is_initialized():
                manager.end_session(session_id)
